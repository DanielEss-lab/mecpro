...
_gaussian = os.environ['MECPRO_HOME']+"/scripts/rung09"
_gprocess = None
...
def _header_template(title, charge, spin, type='', basis='m06/gen', command={},
                     link0={}):
    options = []
    for k, v in link0.items():
        yield '%' + k + '=' + str(v) + '\n'
...
Returns the status code and the running time.
 (Running time to be used for profiling)"""
def run_g09(name, title, charge, spin, molecule, type='', basis='m06/gen',
            command={}, footer=[], link0={}):
    header = _header_template(title, charge, spin, type, basis, command, link0)
    write_input(name, molecule, header, footer)
    start_time = datetime.now()
    global _gprocess
    _gprocess = subprocess.Popen([_gaussian, name])
