...
def verify(params):
    ...
    if params['general']['current_step'] == 0:
        _checkread(params, 0)
        _checkread(params, 1)
...
def _addmix(params, index):
    target = _gettarget(index)
    if params['general']['spinstates'][index] == 1:
...
def _writeroute(f, data):
    ...
    for spin, route in zip(['A', 'B'], data):
        for opt, val in iter(route.items()):
            if val is None:
                f.write('%s: %s\n' % (spin, opt))
...
def write(file, params):
    with open(file, 'w') as f:
        for section, data in iter(params.items()):
            if data is not None:
                f.write(escape("[%s]\n" % section))
