...
def enum(*values):
    def ret(value):
        ...
        if val in values:
            ...
            raise MECPFormatException("Invalid value for enum parameter: " +
                                      value)
    return ret
...
# '''Parses a script section into a dictionary mapping step numbers
# to runnable data'''
...
# A specification for inlined section contents, if any line is prefixed with
# the given string, it will be handled in a certain way.
# prefix:  section
...
def is_valid_file(parse, arg):
    ...
    if not os.path.exists(arg):
        os.parser.error("The file %s does not exist!" % arg)
    else:
