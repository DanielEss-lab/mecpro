...
def vector(a, b):
    return list(map(lambda x: x[1]-x[0], zip(a, b)))
...
def length(vec):
    return functools.reduce(math.hypot, vec)


def unit_vec(vec):
    length = functools.reduce(math.hypot, vec)
...
def mul_mat_vec(mat, vec):
    result = []
    for row in mat:
        result.append(map(lambda x: x[0]*x[1], zip(row, vec)))  # multiply across
    result = map(lambda l: functools.reduce(operator.add, l), result)  # add stuff together
    return list(result)
